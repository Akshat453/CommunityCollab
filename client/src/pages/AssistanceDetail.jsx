    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className={`bg-surface-container-low rounded-3xl p-8 mb-8 border-l-4 ${post.urgency === 'urgent' ? 'border-error' : post.urgency === 'medium' ? 'border-tertiary' : 'border-secondary'}`}>
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${post.status === 'open' ? 'bg-secondary-container text-on-secondary-container' : 'bg-surface-container text-on-surface-variant'}`}>{post.status}</span>
        </div>
          ...
          {post.responses?.length > 0 && (
            <div className="bg-surface-container-low rounded-3xl p-6">
              ...
              <div className="space-y-3">{post.responses.map((r, i) => (
                <div key={i} className="flex items-start gap-3 bg-surface-container rounded-xl px-4 py-3">
                  <img src={r.responder?.avatar_url || 'https://randomuser.me/api/portraits/lego/1.jpg'} alt="" className="w-8 h-8 rounded-full mt-0.5" />
        ...
        <div className="space-y-6">
          <div className="bg-surface-container-low rounded-3xl p-6">
            ...
            <div className="flex items-center gap-3">
              <img src={post.poster?.avatar_url || 'https://randomuser.me/api/portraits/lego/1.jpg'} alt="" className="w-12 h-12 rounded-2xl border-2 border-outline-variant/20" />
